...
const statusColors: Record<string, { bg: string; text: string; dot: string }> = {
  ...
  archived: { bg: "bg-surface-700/50", text: "text-surface-400", dot: "bg-surface-500" },
...
export function StatusBadge({ status, size = "sm", pulse = false }: StatusBadgeProps) {
  ...
  return (
    ...
    >
      <span className={clsx("rounded-full shrink-0", colors.dot, size === "sm" ? "w-1.5 h-1.5" : "w-2 h-2", pulse && "pulse-dot")} />
      {status}
    </span>
  );
}
